...
class ORCA_discrete(Policy):
    ...
    def configure(self, config):
        ...
        logging.info('Orca_danger_dist: %.2f', float(self.env.orca_danger_dist))
        logging.info('Policy: ORCA_discrete without occupancy map')
    ...
    def build_action_space(self, v_pref):
        ...
        """
        if v_pref is None:
            raise AttributeError('v_pref have to be set!')
    ...
    def predict(self, state):
        ...
        # else:
        velocity = np.array((state.self_state.gx - state.self_state.px, state.self_state.gy - state.self_state.py))
        speed = np.linalg.norm(velocity)
        pref_vel = velocity / speed if speed > 1 else velocity

        # reorder actions in ascending order of norm 2 distance with pref_vel
        orca_vel_d = dict()
        
        for action in self.action_space:
            square_dist = (pref_vel[0]-action[0] + 1e-9)**2 + (pref_vel[1]-action[1] + 1e-9)**2 
            # norm_dist = np.linalg.norm((pref_vel))
            orca_vel_d[action] = square_dist
        orca_vel_d = dict(sorted(orca_vel_d.items(), key=lambda item: item[1]))
        max_action = None
        for key, value in orca_vel_d.items():
            ob, reward, done, info = self.env.onestep_lookahead(key)
            max_action = key
            if not (isinstance(info, Collision) or isinstance(info, ORCA_Danger)):
                break
